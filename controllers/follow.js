...
followController.getFollowing = function (req, res, next) {
    models.User.findAll({ attributes: ['id', 'username'], include: [{ model: models.User, as: 'Followers', where: { UserId: req.user.id }, attributes: [] }] })
        .then(function (users) {
...
followController.getFriends = function (req, res, next) {
    req.user.getFollowers({ attributes: ['id', 'username'], include: [{ model: models.User, as: 'Followers', where: { UserId: req.user.id }, attributes: [] }] })
        .then(function (users) {
